#!/bin/bash
#
# nkuwiki wechat.py 定时任务配置脚本
#
# 功能:
# 1. 检查 wechat.py 的 cron 定时任务是否已存在。
# 2. 如果不存在，则添加一个每天凌晨4:05执行一次的任务。
# 3. 任务的输出(包括错误)会被重定向到 logs/cron.log 文件。
#
set -e
...
WECHAT_SCRIPT_PATH="$PROJECT_ROOT/etl/crawler/wechat.py"
...
CRON_JOB_COMMENT="# NKUWiki Wechat Crawler Job"

# 最终要添加到crontab的命令 (每天凌晨4:05执行)
CRON_COMMAND="5 4 * * * $PYTHON_EXEC_PATH $WECHAT_SCRIPT_PATH >> $LOG_FILE 2>&1"
...
echo "============================================="
echo " NKUWiki Wechat Crawler Cron Job Setup"
echo "============================================="
...
if crontab -l 2>/dev/null | grep -Fq -- "$CRON_JOB_COMMENT"; then
    ...
    if [ $? -eq 0 ]; then
        echo "🎉 定时任务添加成功！"
        echo "   现在，wechat.py脚本将每天凌晨4:05自动运行一次。"
