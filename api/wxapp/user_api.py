...
class UserBase(BaseModel):
    ...
    token_count: Optional[int] = Field(0, description="用户Token数量")
    likes_count: Optional[int] = Field(0, description="获得的点赞数")
    favorites_count: Optional[int] = Field(0, description="获得的收藏数")
    followers_count: Optional[int] = Field(0, description="关注者数量")
    following_count: Optional[int] = Field(0, description="关注的用户数量")
    
    @validator('openid')
    def validate_openid(cls, v):
        if not v or not v.strip():
            raise ValueError("微信用户openid不能为空")
        return v.strip()
...
async def get_current_user(
    ...
    """获取当前登录用户信息"""
    api_logger.info(f"Request: GET /users/me with openid={openid}")
    
    try:
        # 通过openid查询用户信息
        api_logger.debug(f"查询用户信息: openid={openid}")
        users = query_records(
            'wxapp_users',
...
async def sync_wxapp_user(
    ...
    try:
        ...
        if existing_users:
            ...
            # 检查可能的唯一键冲突
            existing_by_openid = query_records('wxapp_users', conditions={'openid': user_data.openid})
            if existing_by_openid:
                api_logger.warning(f"检测到可能的openid冲突: {user_data.openid}")
                # 更新这个用户而不是创建
                user_id = existing_by_openid[0]['id']
                success = update_record('wxapp_users', user_id, user_update)
                if success:
                    updated_user = get_record_by_id('wxapp_users', user_id)
                    api_logger.debug(f"通过openid找到并更新用户: {updated_user}")
            else:
                # 尝试插入新记录
