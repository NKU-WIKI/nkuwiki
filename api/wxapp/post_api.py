...
class PostBase(BaseModel):
    """帖子基础信息"""
    openid: str = Field(..., description="发布用户openid")
    ...
    nick_name: Optional[str] = Field(None, description="用户名称")
...
async def create_post(
    ...
    if 'user_id' in post_data:
        post_data['openid'] = post_data.pop('user_id')
    if 'author_name' in post_data:
        post_data['nick_name'] = post_data.pop('author_name')
    if 'author_avatar' in post_data:
        post_data['avatar'] = post_data.pop('author_avatar')
    if 'user_name' in post_data:
        post_data['nick_name'] = post_data.pop('user_name')
    if 'user_avatar' in post_data:
        post_data['avatar'] = post_data.pop('user_avatar')
    
    # 移除数据库中不存在的字段
    if 'wxapp_id' in post_data:
        post_data.pop('wxapp_id', None)
    
    # 创建帖子
    try:
        post_id = insert_record('wxapp_posts', post_data)
        if not post_id:
            api_logger.error("插入帖子失败")
            raise HTTPException(status_code=500, detail="创建帖子失败")
        
        # 获取新创建的帖子
        new_post = get_record_by_id('wxapp_posts', post_id)
        if not new_post:
            api_logger.error(f"无法获取新创建的帖子: ID={post_id}")
            raise HTTPException(status_code=500, detail="创建帖子后获取数据失败")
        
        # 处理JSON字段
        new_post = process_json_fields(new_post)
        
        api_logger.info(f"帖子创建成功: ID={post_id}")
        return create_standard_response(new_post)
    except Exception as e:
        if isinstance(e, HTTPException):
            raise e
        error_details = traceback.format_exc()
        api_logger.error(f"创建帖子异常: {str(e)}\n{error_details}")
