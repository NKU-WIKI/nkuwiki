...
async def toggle_like(post_id: int):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(f"切换点赞状态时发生错误: {exc}")
            return None
        except httpx.HTTPStatusError as exc:
            print(f"切换点赞状态时服务器返回错误状态码: {exc.response.status_code}")
            print(f"响应内容: {exc.response.text}")
            return None
...
async def main():
    ...
    if initial_details is None:
        ...
        return
    print(f"初始点赞数: {initial_details.get('data', {}).get('like_count')}")
    with open("test_results/initial_details.json", "w", encoding="utf-8") as f:
        json.dump(initial_details, f, ensure_ascii=False, indent=4)
    print("初始详情已保存到 test_results/initial_details.json")

    # 2. 执行一次点赞/取消点赞操作
    print("\n2. 执行点赞/取消点赞操作...")
    toggle_result = await toggle_like(TEST_POST_ID)
    action = "点赞" if toggle_result.get('data', {}).get('is_active') else "取消点赞"
    print(f"操作完成: {action}")
    print(f"API返回的新计数值: {toggle_result.get('data', {}).get('count')}")
    with open(f"test_results/toggle_action_result.json", "w", encoding="utf-8") as f:
        json.dump(toggle_result, f, ensure_ascii=False, indent=4)
    print(f"操作结果已保存到 test_results/toggle_action_result.json")

    # 3. 再次获取帖子详情，验证计数是否正确更新
    print("\n3. 再次获取帖子详情以验证...")
    final_details = await get_post_details(TEST_POST_ID)
    print(f"更新后的点赞数: {final_details.get('data', {}).get('like_count')}")
    with open("test_results/final_details.json", "w", encoding="utf-8") as f:
        json.dump(final_details, f, ensure_ascii=False, indent=4)
    print("最终详情已保存到 test_results/final_details.json")

    # 4. 对比结果
    print("\n--- 结果对比 ---")
    initial_count = initial_details.get('data', {}).get('like_count', 0)
    final_count = final_details.get('data', {}).get('like_count', 0)
    api_count = toggle_result.get('data', {}).get('count', -1)

    print(f"初始点赞数: {initial_count}")
    print(f"操作后点赞数 (来自详情接口): {final_count}")
    print(f"操作后点赞数 (来自操作接口): {api_count}")
    ...
    expected_change = 1 if action == "点赞" else -1
