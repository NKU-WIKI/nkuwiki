...
class DeepSeekAgent(Agent):
    ...
    async def areply(self, query: str, context: Context = None) -> Reply:
        """
        异步回复消息
        :param query: 用户消息
        :param context: 上下文
        ...
        if context.type == Context.TYPE_TEXT:
            ...
            # 调用API获取回复
            reply_content = await self._areply_text(session, args)
    ...
    async def _areply_text(self, session, args=None, retry_count=0):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if need_retry:
                    ...
                    return await self._areply_text(session, args, retry_count + 1)
                else:
                    return result
