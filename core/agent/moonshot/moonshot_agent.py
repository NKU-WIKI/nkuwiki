...
logger = register_logger("core.agent.moonshot")
...
class MoonshotSession(Session):
    def __init__(self, session_id, system_prompt=None, model="moonshot-v1-128k"):
        ...
        self.config = Config()
        self.model = model
        self.max_tokens = self.config.get("core.agent.moonshot.max_tokens", 4000)
        self.reset()
...
class MoonshotSessionManager(SessionManager):
    def __init__(self):
        super().__init__(MoonshotSession)


@singleton_decorator
class MoonshotAgent(Agent):
    def __init__(self):
        # 创建配置实例
        self.config = Config()
        
        # 获取配置
        self.api_key = self.config.get("core.agent.moonshot.api_key")
        
        # 获取模型配置
        self.model = self.config.get("core.agent.moonshot.model", "moonshot-v1-128k")
        
        # 初始化参数
        self.temperature = self.config.get("core.agent.moonshot.temperature", 0.3)
        self.top_p = self.config.get("core.agent.moonshot.top_p", 1.0)
        
        # API基础URL
        self.api_base = self.config.get("core.agent.moonshot.base_url", "https://api.moonshot.cn/v1/chat/completions")
        
        # 会话管理
        self.session_manager = MoonshotSessionManager()
    ...
    def reply(self, query, context: Context = None) -> Reply:
        ...
        if context.type == Context.TYPE_TEXT:
            logger.debug(f"[Moonshot] 接收到文本请求: {query}")
            
            # 检查API参数是否已配置
            if not self.api_key:
                return Reply(ReplyType.ERROR, "请先配置Moonshot API参数")
            ...
            # 处理回复
            logger.debug(
                f"[Moonshot] 会话ID={session_id}, 内容={reply_content['content']}, tokens={reply_content['completion_tokens']}"
            )
            ...
            if reply_content["completion_tokens"] == 0 and len(reply_content["content"]) > 0:
                ...
                if self.show_user_prompt:
                    logger.debug(f"[Moonshot] {query} \n {reply_content['content']}")
                return Reply(ReplyType.TEXT, reply_content["content"])
            else:
                logger.error(f"[Moonshot] 回复使用了0个token: {reply_content}")
                return Reply(ReplyType.ERROR, reply_content["content"])
        else:
    ...
    def _reply_text(self, session, args=None, retry_count=0):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"[Moonshot] 请求失败: 状态码={response.status_code}, "
                ...
                if response.status_code >= 500:
                    # 服务器错误，需要重试
                    logger.warning(f"[Moonshot] 进行第{retry_count+1}次重试")
            ...
            logger.exception(f"[Moonshot] 发生异常: {e}")
            need_retry = retry_count < 2
