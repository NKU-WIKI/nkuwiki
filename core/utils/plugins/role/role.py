...
class RolePlay:
    def __init__(self, agent, sessionid, desc, wrapper=None):
        self.agent = agent
        self.sessionid = sessionid
        self.wrapper = wrapper or "%s"  # 用于包装用户输入
        self.desc = desc
        self.agent.sessions.build_session(self.sessionid, system_prompt=self.desc)

    def reset(self):
        self.agent.sessions.clear_session(self.sessionid)

    def action(self, user_action):
        session = self.agent.sessions.build_session(self.sessionid)
...
class Role(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if desckey is not None:
            ...
            self.roleplays[sessionid] = RolePlay(agent, sessionid, clist[1], "%s")
