...
class LinkSummary:
    ...
    def summary_file(self, file_path: str, app_code: str):
        ...
        App().logger.info(f"[LinkSum] file summary, app_code={app_code}")
    ...
    def summary_url(self, url: str, app_code: str):
        ...
        App().logger.info(f"[LinkSum] url summary, app_code={app_code}")
    ...
    def summary_chat(self, summary_id: str):
        ...
        if res.status_code == 200:
            res = res.json()
            App().logger.debug(f"[LinkSum] chat open, res={res}")
            if res.get("code") == 200:
                data = res.get("data")
                return {
            ...
            res_json = res.json()
            App().logger.error(f"[LinkSum] summary error, status_code={res.status_code}, msg={res_json.get('message')}")
            return None

    def _parse_summary_res(self, res):
        if res.status_code == 200:
            res = res.json()
            App().logger.debug(f"[LinkSum] summary result, res={res}")
            if res.get("code") == 200:
                data = res.get("data")
                return {
    ...
    def base_url(self):
        return Config().get("linkai_api_base", "https://api.link-ai.tech")

    def headers(self):
        return {"Authorization": "Bearer " + Config().get("linkai_api_key")}
    ...
    def check_file(self, file_path: str, sum_config: dict) -> bool:
        ...
        if (sum_config.get("max_file_size") and file_size > sum_config.get("max_file_size")) or file_size > 15000:
            App().logger.warn(f"[LinkSum] file size exceeds limit, No processing, file_size={file_size}KB")
            return False

        suffix = file_path.split(".")[-1]
        support_list = ["txt", "csv", "docx", "pdf", "md", "jpg", "jpeg", "png"]
        if suffix not in support_list:
            App().logger.warn(f"[LinkSum] unsupported file, suffix={suffix}, support_list={support_list}")
            return False
    ...
    def check_url(self, url: str):
        ...
        for black_url_prefix in black_support_list:
            if url.strip().startswith(black_url_prefix):
                App().logger.warn(f"[LinkSum] unsupported url, no need to process, url={url}")
                return False
